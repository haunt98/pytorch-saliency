...
class IterativeSaliency:
    def __init__(
        ...
        self.saliency_loss_calc = SaliencyLoss(
            self.black_box_fn,
            area_loss_coef=11,
    ...
    def get_saliency_maps(self, _images, _targets, iterations=None, show=False):
        ...
        if self.cuda:
            _mask = nn.Parameter(
                torch.Tensor(_images.size(0), 2, self.mask_resolution, self.mask_resolution).fill_(0.5).cuda()
            )
        else:
            _mask = nn.Parameter(
                torch.Tensor(_images.size(0), 2, self.mask_resolution, self.mask_resolution).fill_(0.5)
            )
