...
def calc_area_loss(mask, power=1.0):
    if power != 1:
...
def apply_mask(
    ...
    if preserved_imgs_noise_std > 0:
        images = images + Variable(
            tensor_like(images).normal_(std=preserved_imgs_noise_std),
            requires_grad=False,
        )
    if bypass > 0:
    ...
    if blurred_version_prob > 0.0:  # <- it can be a scalar between 0 and 1
        ...
        if cuda:
            ...
            when = Variable(
                (torch.Tensor(n, 1, 1, 1).uniform_(0.0, 1.0) < blurred_version_prob).float(),
                requires_grad=False,
            )
...
def test():
    ...
    im = Variable(
        torch.Tensor(
            np.expand_dims(
                np.transpose(
                    np.array(Image.open(os.path.join(os.path.dirname(__file__), "test.jpg"))),
                    (2, 0, 1),
                ),
                0,
            )
    ...
    print()
    for pres in [1.0, 0.5, 0.1]:
        print("Mask strength =", pres)
        for e in range(5):
            m = Variable(
                torch.Tensor(1, 3, im.size(2), im.size(3)).fill_(pres),
                requires_grad=True,
            )
            res = apply_mask(im, m)
