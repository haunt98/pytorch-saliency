...
def smoothing_dict_update(main, update, smooth_coef):
    for k, v in list(update.items()):
        if main.get(k) is None:
            ...
            main[k] = smooth_coef * main[k] + (1.0 - smooth_coef) * v
    return main
...
class NiceTrainer:
    def __init__(
        ...
        assert isinstance(
            train_dts, torch_data.DataLoader
        ), "train_dts must be an instance of torch.utils.data.DataLoader"
        ...
        assert val_dts is None or isinstance(
            val_dts, torch_data.DataLoader
        ), "val_dts must be an instance of torch.utils.data.DataLoader or None"
        self.val_dts = val_dts
        if modules is not None:
            if not hasattr(modules, "__iter__"):
                modules = [modules]
        assert modules is None or all(
            [isinstance(x, torch.nn.Module) for x in modules]
        ), "The list of modules can only contain instances of torch.nn.Module"
    ...
    def _main_loop(self, is_training, steps=None, allow_switch_mode=True):
        ...
        if steps is not None:
            print(
                WARN_TEMPLATE % "Num steps is not fully supported yet! (fix it!)"
            )  # todo allow continue and partial execution
        if not is_training:
        ...
        if allow_switch_mode and self._is_in_train_mode != is_training:
            if self.set_trainable is not None:
                ...
                if is_training:
                    print(
                        WARN_TEMPLATE
                        % "could not set the modules to the training mode because neither set_trainable nor modules were provided, assuming already in the training mode"
                    )
                    self._is_in_train_mode = True
                else:
                    raise ValueError(
                        "cannot set the modules to the eval mode because neither set_trainable nor modules were provided"
                    )
        ...
        for batch in dts_iter:
            ...
            smoothing_update.update(
                {e: to_number(self.pt_store[e]) for e in self.printable_vars if e not in self._core_info_vars}
            )
            ...
            # calculate
            batches_per_sec = 1.0 / (smoothed_printable_vars["comp_time"] + smoothed_printable_vars["data_time"])
            batches_done += 1
            steps_done_here += 1
...
def auto_norm(im, auto_normalize=True, auto_fix=True):
    ...
    min_v = np.min(im)
    max_v = np.max(im)
    if 0 < np.max(im) - np.min(im) < 11.0 and auto_normalize:  # something wrong, perform normalisation
        return ((im - (min_v + max_v) / 2.0) / (max_v - min_v)) * 255 + 255 / 2.0
    return im
...
def _caclulate_batch_top_n_hits(probs, labels, n, avg_preds_over):
    ...
    if n == 1:
        ...
        return np.mean(
            (
                np.sum(
                    np.cumsum(np.argsort(probs, axis=1) == np.expand_dims(labels, 1), 1),
                    1,
                )
                <= n
            ).astype(np.float32)
