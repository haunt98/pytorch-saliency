...
def get_black_box_fn(model_zoo_model=resnet50, cuda=True, image_domain=(-2.0, 2.0)):
...
class SaliencyModel(Module):
    ...
    def minimialistic_restore(self, save_dir):
        assert (
            self.fix_encoder
        ), "You should not use this function if you are not using a pre-trained encoder like resnet"
    ...
    def minimalistic_save(self, save_dir):
        ...
        for name, module in list(self._modules.items()):
            if (
                module is self.encoder
            ):  # we do not want to restore the encoder as it should have its own restore function
                continue
    ...
    def get_trainable_parameters(self):
        ...
        return (
            set(all_params)
            - set(unwanted)
            - (set(self.selector_module.parameters()) if self.allow_selector else set([]))
        )
    ...
    def forward(self, _images, _selectors=None, pt_store=None, model_confidence=0.0):
        ...
        if self.use_simple_activation:
            return (
                torch.unsqueeze(torch.sigmoid(saliency_chans[:, 0, :, :] / 2), dim=1),
                exists_logits,
                out[-1],
            )
...
class SaliencyLoss:
    def __init__(
        ...
        preserver_loss_coef=0.3,
        num_classes=1000,
        area_loss_power=0.3,
        preserver_confidence=1,
        destroyer_confidence=5,
        **apply_mask_kwargs,
    ):
    ...
    def get_loss(self, _images, _targets, _masks, _is_real_target=None, pt_store=None):
        ...
        if _is_real_target is None:
            _is_real_target = Variable(tensor_like(_targets).fill_(1.0))
        destroyed_images = apply_mask(_images, 1.0 - _masks, **self.apply_mask_kwargs)
