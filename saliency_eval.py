...
def load_image_as_variable(path, cuda=False):
    ...
    return to_batch_variable(
        np.expand_dims(np.transpose(np.array(Image.open(path)), (2, 0, 1)), 0) / 255.0 * 2 - 1.0,
        4,
        cuda=cuda,
    ).float()
...
def test(cuda=True):
    ...
    for e in range(20 if cuda else 2):
        f(
            np.random.randn(32, 3, 224, 224),
            np.random.uniform(0, 100, size=(32,)).astype(np.int),
            6,
        )
    print("Images per second:", 32.0 * (20 if cuda else 2) / (time.time() - t))
